...
class MultiHeadAttention:
    ...
    def __init__(self, d_model: int, num_heads: int) -> None:
        ...
        logger.info(
            "[%s] MultiHeadAttention 已创建: d_model=%d, num_heads=%d",
            tid, d_model, num_heads
        )
    ...
    def forward(self, x: np.ndarray, mask: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
        ...
        if mask is not None:
            # mask 可能为 (seq_len, seq_len)，需要扩展维度使其可广播至 (batch_size, num_heads, seq_len, seq_len)
            if mask.ndim == 2:
        ...
        output_concat = output_heads.transpose(0, 2, 1, 3).reshape(batch_size, seq_len, self.d_model)
