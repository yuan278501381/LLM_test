    ...
    def get_historical_benchmarks() -> list[dict]:
        ...
        return [
            ...
            },
            {
                ...
                "status": "Chinchilla 设定下的近似算力最优",
                "note": "在该论文研究范围内接近 D ≈ 20N；不能跨数据与架构直接保证效果",
            },
            {
                "name": "LLaMA-1 (2023)",
                "params": 7e9,
...
class DataMixtureEngine:
    """
    教学用预训练语料配比 (Data Mixture) 与清洗流水线示意。

    公开模型通常不会披露可复现的完整数据配方，因此这里的比例只用于控制变量实验，
    不对应 LLaMA、DeepSeek 或 FineWeb-Edu 的官方组成。
    """

    @staticmethod
    def get_mixtures() -> dict[str, dict[str, float]]:
        """返回总和为 100% 的教学假设配比；不得引用为任何真实模型配方。"""
        return {
            "教学示意 A：通用均衡型": {
                "通用网页": 50.0,
                "源代码与技术文档": 20.0,
