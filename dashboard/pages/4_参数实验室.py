...
教学用微观监控实验室：四宫格图、单步调试、快照对比与超参数动态调整。
"""
...
render_hero_header(
    title="全参数微观实验室",
    subtitle="教学用四宫格遥测台 · 支持逐步训练 · 参数快照回看与 A/B 对比",
...
with grid_c1:
    ...
    with st.expander("[HOW TO READ // 读图指南] 空间决策流形与分界线", expanded=False):
        st.markdown(
            ...
            * **[观察目标]**：黑色边界应随训练数据与损失变化；本次训练集准确率需结合测试集、随机种子和类别分布解释，95% 不是通用合格线。
            """
        )
    ...
    with st.expander("[HOW TO READ // 读图指南] 逐层权重参数直方图", expanded=False):
        st.markdown(
            ...
            * **[如何解读]**：权重分布由初始化、优化器、正则化和任务共同决定；是否近似钟形或以 0 为中心都不是健康训练的充分条件。
...
with grid_c2:
    ...
    with st.expander(
        ...
        st.markdown(
            ...
            * **[CONVERGED // 当前运行收敛]**：Loss 整体下降且 Accuracy 整体上升；随机小批训练不保证逐步单调，也不等于测试集最优。
    ...
    with st.expander("[HOW TO READ // 读图指南] 反向传播梯度流分布", expanded=False):
        st.markdown(
            """
            * **横轴**：梯度数值范围；**纵轴**：参数个数。
            * **[如何解读]**：关注各层梯度是否长期接近数值零、非有限值或与参数尺度严重失配；不存在适用于所有网络的固定“健康区间”。
...
with st.expander(
    ...
    st.markdown(
        ...
        | **$\\Omega(W)$** | **权重复杂度惩罚项** | • **L1 正则 (Lasso, $\\sum |w_i|$)**：产生恒定梯度的收缩惩罚，促使非关键特征的权重向 0 收缩（普通次梯度更新在浮点数上不保证精确截断为纯零，精确稀疏通常需要配合近端算子/软阈值处理）；<br>• **L2 正则 (Ridge, $\\frac{1}{2}\\sum w_i^2$)**：惩罚权重平方和，对绝对值大的权重施加更强的回拉力，使权重普遍变小、平滑（在标准 SGD 下等价于权重衰减，但在自适应优化器如 Adam 下不等价，需使用 AdamW 解耦；L2 约束整体范数，并不保证各分量均匀分布）。 |
        ...
        * **经验法则**：在典型神经网络常规训练中，每一步更新的量 $\\|\Delta W\\|$ 与当前权重大小 $\\|W\\|$ 的比值通常在 **$10^{-3} \\approx 0.001$** 数量级附近作为参考启发式经验；若远低于 $10^{-5}$ 则学习过慢，若远高于 $10^{-1}$ 则易发散（具体受优化器、Warmup 阶段与权重衰减策略影响）。
        """
    )
