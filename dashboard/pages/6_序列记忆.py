...
with st.expander("[HOW TO READ // 读图指南] 时间步传递与长程遗忘瓶颈时间步传递与长程遗忘瓶颈", expanded=False):
    st.markdown(
        """
...
with st.expander("[GROWTH GUIDE // 成长指南] RNN 核心公式拆解与序列时序名词通俗全解", expanded=False):
    st.markdown(
        ...
        |:---:|:---:|:---|
        | **$x_t$** | **当前时间步输入词向量 (Current Input)** | 当前时刻读到的新单词（例如“猫咪”的向量）。 |
        | **$W_{ih}$** | **输入-隐层权重矩阵 (Input-to-Hidden)** | 负责把当前新单词的特征提取转换并存入记忆。 |
        | **$h_{t-1}$** | **上一时刻的历史记忆 (Previous Memory)** | 截止到上一个词为止，大脑里已经记住的全部故事背景。 |
        | **$W_{hh}$** | **隐层-隐层递归权重矩阵 (Hidden-to-Hidden)** | **记忆过滤器**。决定把上一时刻的历史记忆保留多少、按什么方式融入当前时刻。 |
        | **$b_h$** | **循环偏置常数 (Bias)** | 记忆状态的基础偏移。 |
        | **$\\tanh$** | **双曲正切激活函数** | 把加权后的总记忆平滑压缩到 $[-1, 1]$ 之间，防止记忆数值越滚越大发生数值爆炸。 |
        | **$h_t$** | **当前时刻综合新记忆 (New Memory)** | 融合了“新词 $x_t$”与“旧记忆 $h_{t-1}$”后的最新大脑状态，准备传给下一个时间步 $t+1$。 |
