...
with col_wave_plot:
    ...
    with st.expander("[HOW TO READ // 读图指南] 微观时域振动波形图", expanded=False):
        st.markdown(
            ...
            * **波形高低**：物理振幅绝对值代表声压强弱（注意：人类主观感知的响度还受频率与等响曲线影响，低频与极高频在相同物理振幅下主观感知响度较低）。
            """
        )
...
with st.expander(
    ...
    st.markdown(
        ...
        * **横轴是时间**（从前奏到副歌）；**纵轴是音高频率**（从低音贝斯到高音女高音）；**颜色越亮代表该时刻该音高的能量越强**！
        * 现代端到端语音识别（如 OpenAI Whisper）将分帧提取的对数梅尔谱图输入 1D/2D 卷积前置干线并送入 Encoder-Decoder Transformer 架构，直接完成跨时间步的声学特征建模与文本自回归转写解码。
        """
    )
