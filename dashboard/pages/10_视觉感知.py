...
with col_clip_mat:
    ...
    with st.expander("[HOW TO READ // 读图指南] CLIP 图文跨模态对齐矩阵", expanded=False):
        st.markdown(
            ...
            * **[观察目标 // 批内对齐]**：对角线相似度高于非对角线，表示这批配对样本被区分；它本身不能证明对未见类别的零样本分类能力。
            """
        )
...
with st.expander(
    ...
    st.markdown(
        """
        ### 0. 核心公式逐字拆解：2D 空间卷积滤波（深度学习互相关约定）
        $$(I * K)(i, j) = \\sum_{m=-1}^1 \\sum_{n=-1}^1 I(i+m, j+n) \\cdot K(m, n)$$
        *(工程约定说明：严格数学卷积定义需将核旋转 180° $K(-m, -n)$；深度学习中由于卷积核参数是端到端学习得到的，现代框架如 PyTorch/TensorFlow 普遍约定采用未翻转核的互相关 Cross-Correlation 计算)*
        ...
        | **$* (星号)$** | **2D 空间滑动内积** | 运算符号 | 卷积核在整张图片上从左往右、从上往下像扫描仪一样滑动，对应位置相乘并累加。 |
        ...
        * **目标**：“猫咪的照片”和“写着'一只猫'的文字”，在空间里的距离被拉得极近；而“猫咪照片”和“写着'汽车'的文字”被推得极远！
        ...
        | **$\\tau$** | **温度系数 (Temperature)** | 通常 $\\tau=0.07$（极低温），让模型对"对不对"这件事更加严格果断（拉大正负样本得分差距）。 |
        | **$N$** | **批次内正样本对数量** | 每个 Batch 中有 $N$ 对正确图文对。 |
        | **分子 $e^{S_{ii}/\\tau}$** | **正样本的匹配得分** | 正确图文对的相似度经过指数放大后的值。 |
        | **分母 $\\sum_j e^{S_{ij}/\\tau}$** | **所有候选配对的得分总和** | 正确配对 + 所有错误配对的总得分和。InfoNCE 的目标就是让正确配对在分母中的占比越来越大（趋向 1.0）。 |
        """
    )
