...
if final_acc < 0.90 or final_loss > 0.5:
    if dataset_name.lower() in ["xor", "moons", "circles", "spiral"]:
        st.warning(
            ...
            f"• **为什么画不准？** 当前选用的是 **{dataset_name.upper()} 非线性流形**。单神经元的数学本质是单个线性超平面（`w₁·x₁ + w₂·x₂ + b = 0`），只能在输入空间中确定一条直线决策边界。明斯基（Minsky）与帕珀特（Papert）在 1969 年指出：单层线性感知机在不进行非线性特征升维时，无法实现异或（XOR）等线性不可分流形的正确划分。\n\n"
            f"• **如何提升分类表现？**\n"
            f"  1. **体验线性可分分类**：在左侧控制台将【分布类型】切换为 **Blobs (高斯聚类线性分布)**，或直接点击左上方 **【关卡 1 初出茅庐】**；\n"
            f"  2. **攻克非线性难题**：点击左侧导航栏前往 **M02·多层网络 (Multi-Layer Network)**，引入隐藏层神经元实现特征空间的非线性折叠！"
        )
    elif act_meta.id != "Sigmoid":
        st.warning(
            f"**[DIAGNOSTIC HINT // 智能诊断：激活函数特性影响]**\n\n"
            f"当前模型在线性数据集上未达最优收敛（当前准确率：**{final_acc:.1%}**，最终损失：**{final_loss:.4f}**）。\n\n"
            f"• **为什么准确率受限？** 当前激活函数选用了 **{act_meta.label}**。可微逻辑神经元的标准输出应是将实数线性加权值平滑压缩至 `(0, 1)` 概率区间的 **Sigmoid** 激活函数。使用 ReLU 时，负向区域导数完全置零，可能引发神经元死区（Dying ReLU）导致参数停止更新。\n\n"
            f"• **如何调整参数？**\n"
            f"  1. 在左侧控制台将【激活函数】切换为 **Sigmoid (S型激活函数)**；\n"
            f"  2. 将【学习率】设为 **0.05 ~ 0.2** 推荐区间，观察模型在线性可分点集上的平稳收敛。"
        )
    elif lr > 1.0:
        st.warning(
            f"**[DIAGNOSTIC HINT // 智能诊断：学习率过大导致震荡]**\n\n"
            f"当前模型发生剧烈翻滚震荡（当前准确率：**{final_acc:.1%}**，最终损失：**{final_loss:.4f}**）。\n\n"
            f"• **为什么损失震荡？** 当前学习率过大（**LR = {lr}**），导致参数每次更新迈出的步长过大，在损失盆地两侧剧烈跳跃，无法沉降至平稳极小值区域。\n\n"
            f"• **如何调整参数？**\n"
            f"  1. 在左侧控制台将【学习率】下调至 **0.05 ~ 0.2**；\n"
            f"  2. 重新观察右侧损失曲线，将平滑单调下降，分类准确率逐步攀升。"
        )
    else:
        st.warning(
