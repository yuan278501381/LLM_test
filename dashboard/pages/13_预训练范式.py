...
with st.expander(
    ...
    st.markdown(
        ...
        |:---:|:---:|:---:|:---|
        | **$L(N, D)$** | **预测预训练交叉熵损失 (Loss)** | 连续实数 | 评估模型在训练分布上的下一个 Token 预测负对数似然损失（损失越低，语言建模困惑度越低；但较低损失不直接等价于综合推理与事实真理性智能）。 |
        ...
        | **$N$** | **模型非嵌入参数量 (Parameters)** | 比如 $7\\text{B} = 7 \\times 10^9$ | **神经网络参数容量**。参数量越大，能够拟合和记忆的函数复杂度与特征表示空间上限越高。 |
        | **$D$** | **训练 Token 总数 (Data Tokens)** | 比如 $2\\text{T} = 2 \\times 10^{12}$ | **训练集消耗的 Token 序列总量**。数据量越大，模型覆盖的词法、语法与上下文统计规律越丰富。 |
        ...
        ### 1. 为什么 Chinchilla 存在 20:1 与动态比例两种表述？
        * **等比例扩展启发式 (Approach 1 & 2)**：假定模型参数 $N$ 与训练数据 $D$ 严格同比例增长（$a \\approx b \\approx 0.5$），70B 参数模型按固定比例配置 1.4T tokens（即 $20:1$ 比例）；
        * **非对称参数化拟合公式 (Approach 3, Table A3)**：拟合参数 $\\alpha=0.3392, \\beta=0.2849 \\implies a=0.4565, b=0.5435$。因为数据增长指数略高于参数增长指数，在超大规模算力预算下，最优数据/参数比例会随总算力按幂律持续提升（在 $N_{\\text{opt}}=70\\text{B}$ 时理论算力最优分配对应 $D_{\\text{opt}} \\approx 4.59\\text{T}$，比例约为 $65.6:1$）。
        """
    )
