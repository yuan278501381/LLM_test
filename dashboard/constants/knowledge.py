...
PRESETS_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "梯度消失困境复现 (Vanishing Gradient)": {
        "desc": "4 层 Sigmoid + Random 初始化导致前端梯度归零，网络陷入停滞",
        "dataset": "circles",
        "n_samples": 300,
        "noise": 0.10,
        "n_layers": 4,
    ...
    "梯度消失拯救对照 (ReLU + He 对照组)": {
        "desc": "同为 4 层网络，切换为 ReLU + He 初始化与 Adam 优化器，见证梯度恢复畅通与收敛",
        "dataset": "circles",
        "n_samples": 300,
        "noise": 0.10,
        "n_layers": 4,
...
MODERN_LLM_ARCH: dict[str, ArchitectureMeta] = {
    ...
    "GQA": ArchitectureMeta(
        ...
        impact="在 Ainslie et al. (2023) 实验中以接近 MHA 的质量表现，成倍数级压缩了推理时的 KV-Cache 显存占用与内存带宽开销。",
        example="Llama-3-70B (8 个 KV 头服务 64 个 Q 头) 与高并发吞吐场景的主流架构。",
    ),
    "SwiGLU": ArchitectureMeta(
        ...
        impact="在 Shazeer (2020) 实验中，在计算量相当的前提下取得了比标准 GELU/ReLU 更优的验证集困惑度与收敛表现。",
        example="PaLM, LLaMA-2/3 等现代底层引擎采用的标准非线性模块。",
    ),
    "KV-Cache": ArchitectureMeta(
        ...
        impact="避免了历史 token 的重复投影计算；单步生成与历史长度为 t 的缓存做注意力计算仍需 O(t) 时间，总生成 N 个 token 的计算量由无缓存的 O(N³) 降至 O(N²)，显存占用随序列长度线性增长 O(t)。",
...
MULTIMODAL_ARCH: dict[str, ArchitectureMeta] = {
    "Conv2D": ArchitectureMeta(
        ...
        desc="通过小尺寸滑动窗口在二维空间图像上局部相乘求和（深度学习中通常约定为未翻转核的互相关实现），具备平移不变性与局部特征聚集能力。",
...
TRAINING_ARCH: dict[str, ArchitectureMeta] = {
    ...
    "LoRA": ArchitectureMeta(
        ...
        formula="h = x W_0 + \\frac{\\alpha}{r} (x A) B, \\quad A \\in \\mathbb{R}^{d_{\\text{in}} \\times r}, B \\in \\mathbb{R}^{r \\times d_{\\text{out}}}",
        desc="冻结预训练主干权重，在矩阵旁路引入两个极小秩的低秩分解矩阵 A 和 B 进行梯度更新，推理时可直接合并消除额外延迟。",
        impact="在特定微调任务下可将可训练参数量压缩 95% 以上，显著减少优化器状态显存（但冻结基座仍需驻留显存；单卡微调超大模型常需结合 4-bit 量化等 QLoRA 机制）。",
    ...
    "Perplexity": ArchitectureMeta(
        ...
        desc="衡量自回归语言模型在预测测试集文本时的平均交叉熵指数，数值越低代表模型对测试集概率分布的预测不确定性越小。",
        impact="大语言模型预训练阶段最基础的内在预测能力量化指标（数值受 tokenizer 词表与分词粒度直接影响，不能跨不同 tokenizer 绝对对比，也不直接代表推理或对齐能力）。",
...
AI_TRAPS_REGISTRY: dict[str, dict[str, Any]] = {
    "AttentionSink": {
        "id": "AttentionSink",
        "name": "注意力汇聚 (Attention Sink)",
        "category": "长上下文与滑动窗口",
        "phenomenon": "长文本流式推理中，观察到初始 Token 往往出现强注意力汇聚，保留其 KV 缓存能显著恢复滑动窗口下的注意力表现（Xiao et al., 2023）。",
        "cause": "机制直觉与可能解释：Softmax 权重和恒为 1 的归一化约束，在缺乏显式全局聚合机制时可能促使自注意力层将冗余注意力分配给最初的 Token（注：Softmax 是必要约束但非充分因果解释）。",
        "failure_case": "滑动窗口推理中若直接丢弃初始 Token，模型内部激活值分布可能产生严重畸变，导致困惑度 (PPL) 显著恶化。",
        "best_practice": "保留少量初始 Token 作为固定 Attention Sinks (StreamingLLM 方案)，有助于稳定长文本流式生成时的注意力分布与显存开销。",
    },
    "LostInTheMiddle": {
        "id": "LostInTheMiddle",
        "name": "迷失在中间 (Lost in the Middle)",
        "category": "长上下文检索偏置",
        "phenomenon": "在长上下文多段落检索中，位于文档正中间位置的关键信息召回准确率往往低于开头和结尾（Nelson F. Liu et al., TACL 2024）。",
        "cause": "自注意力机制的自回归位置偏置与注意力稀释，大量中间干扰段落摊薄了目标信息的注意力权重。",
        "failure_case": "将检索到的多篇文档按随机或未经重排的顺序拼接，核心证据若落入中间盲区，可能导致模型漏召回或产生幻觉。",
        "best_practice": "引入重排算法 (Rerank) 将高相关性核心证据置顶，或精简上下文并配合结构化分段索引，同时需权衡时序依赖完整性。",
    },
    "ContextCompaction": {
        ...
        "failure_case": "每轮用 LLM 总结上一轮历史，连续多轮后函数名、关键变量与行号可能被泛化抹杀，模型开始凭空虚构接口。",
    ...
    "ReversalCurse": {
        ...
        "category": "自回归因果概率单向性",
        "phenomenon": "模型能熟练回答'A 的主要成果是 B'，但直接提问'B 的主要作者是谁'时却频繁回答不确定或错误关联（Berglund et al., ICLR 2024）。",
        "cause": "自回归 Transformer 建模的是单向条件概率 P(B | A, 关系)，未在参数内部自发建立双向对称知识表示。",
        "failure_case": "盲目依赖模型隐式反向推导复杂关系链，导致知识图谱提取出现单向事实性遗漏。",
        "best_practice": "在微调语料中显式加入双向配对样本，在当前上下文中显式提供双向关系定义，或结合外部结构化知识库。",
    },
    "ClaudeCode2026": {
        "id": "ClaudeCode2026",
        "name": "2026 Claude Code 事故复盘 (Harness Engineering)",
        "category": "Agent 脚手架工程设计",
        "phenomenon": "2026 年 3~4 月 Claude Code 出现偶发长代码截断、复杂推理深度不足等问题，用户一度怀疑模型底模被降级。",
        "cause": "Anthropic 官方技术复盘证实为 Client/Harness 层的 3 项工程失误：默认思考预算调低、闲置会话缓存清理 Bug、系统提示词简短要求过度约束。",
        "failure_case": "会话清理优化误在每轮抹除思考链历史，导致模型在长任务中表现出重复探索；简短提示词导致部分扩展评测集观察到约 3% 性能下降。",
    ...
    "AgentCircuitBreaker": {
        ...
        "phenomenon": "Agent 遇到微小路径错误陷入连续重试死循环消耗预算，或读取外部不可信数据时遭遇隐藏指令干扰。",
        "cause": "缺少工具调用幂等熔断机制；未对不可信外部数据与系统控制平面进行物理隔离。",
        "failure_case": "读取包含恶意指令的第三方文件导致 Agent 尝试执行非预期操作，或因死循环耗尽 Token 预算。",
