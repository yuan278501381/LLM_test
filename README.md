...
当前代码收集 **374 项测试并全部通过**，`nn_core + datasets` 分支覆盖率为 **94.74%**（远超 80.0% 门禁标准）。
...
- Ruff 覆盖 dashboard 与 nn_core，Pyright 严格类型检查，本地 DevOps 统一控制台（Pre-commit + `scripts/devops.py test`）统一调度 7 大阶段（Format、Lint、Typecheck、Diff、374 项单测 / 94.74% 分支覆盖率、18 页面 Chromium 真实端到端导航与部署幂等性门禁）。

当前基线已更新为 **374 passed / 94.74%**。覆盖率只表示受执行代码比例，不能代替断言质量与教学事实审校。
