...
详细问题、修复顺序和验收断言见 [Sol-中实施交接清单](HANDOFF_SOL.md)；长期不变量和后续验收记忆见 [项目记忆](PROJECT_MEMORY.md)。

## ✨ 学习路径与 M00-M16 里程碑总览

本项目目前提供 **M00-M16 共 17 个渐进式里程碑**，覆盖必要数学、线性分类、现代模型组件、评估方法与强化学习入门。课程数量不代表知识体系已经完整；完整的已实现/部分实现/仅规划状态见 [课程地图](CURRICULUM_MAP.md)：
