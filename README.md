...
> **当前状态：AI 科学性综合审计与全量整改全部通过。** P0 核心损失函数与准确率契约彻底重构（BCE/BCEWithLogits/CCE 严格概率与实数域输入校验、移除无条件截断、新增数值稳定 `CategoricalCrossEntropyWithLogits`），18 道形成性测验完成科学性修订并挂载 `sources/conditions/limitations` 审计元数据，72 条 Claims 彻底解除机械索引分配并与权威文献建立一对一语义绑定，后训练遥测去除了绝对化宣传并明确标明教学假设情景，全套 7 阶段 DevOps 门禁全绿。

当前代码收集 **392 项测试并全部通过**，`nn_core + datasets` 分支覆盖率为 **94.49%**（远超 80.0% 门禁标准）。

本次整改与前沿架构拓展建立了以下可审计基础：

- M00-M17 全部进入机器可读无环课程依赖图；每课公开诊断题、最小实验、反例实验、形成性评价和通过标准。
- 深度落地 FlashAttention-2 核心分块与 Online Softmax (M08)、PagedAttention 显存分页与前缀共享 (M09)、MoE 混合专家稀疏门控路由与负载均衡 (M13) 纯白盒算法，以及全站通用的双实验并排消融差分对比套件。
- 18 页当前登记 72 个 result ID；M00-M17 全部完成与独立文献或计算协议的一对一精准语义绑定（经独立 fixture 审核，杜绝循环自证）。
- 核心契约包含输入非空/有限值/形状校验、数值稳定 Log-Sum-Exp 多分类交叉熵、显式损失函数分类准确率契约。
- Ruff 覆盖 dashboard 与 nn_core，Pyright 严格类型检查，本地 DevOps 统一控制台（Pre-commit + `scripts/devops.py test`）统一调度 7 大阶段（Format、Lint、Typecheck、Diff、392 项单测 / 94.49% 分支覆盖率、18 页面 Chromium 真实端到端导航与部署幂等性门禁）。

当前基线已更新为 **392 passed / 94.49%**。覆盖率只表示受执行代码比例，不能代替断言质量与教学事实审校。
