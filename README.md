...
> **当前状态：工程门禁通过，但当前版本尚未通过教学内容复审。** 2026-08-22 的通过结论仅覆盖当时的 M00-M16；当前 `main` 已新增 M17。M17 含无来源比例、规则生成曲线冒充经验结果、伪造事故准确率/延迟和错误引用等发布阻断问题，修复并重新验收前不得称为教学可信版本。

当前代码收集 **343 项测试并全部通过**，`nn_core + datasets` 分支覆盖率为 **95.18%**。这一结果同时说明：测试可以证明代码按当前规则运行，却不能自动证明规则、页面解释和引用真实。详细问题与 Sol-中任务见 [2026-08-23 第二阶段交接清单](HANDOFF_SOL_2026-08-23.md)。

本次整改建立了以下可审计基础：

- M00-M17 已进入机器可读无环课程依赖图；每课公开诊断题、最小实验、反例实验、形成性评价和通过标准。
- 18 页当前登记 72 个 result ID；M00-M16 的证据基线已验收，M17 的四条通用模板 claim 尚不能支持页内具体数字，必须按第二阶段清单重建。
...
- 2026-08-22 浏览器抽查确认当时 17 页无 Streamlit 页面异常、导航和播放器行为符合门禁；该记录不覆盖后来新增的 M17，当前版本需要完成 18 页复验。

2026-08-22 历史基线为 **322 passed / 95.08%**；当前基线已更新为 **343 passed / 95.18%**。覆盖率只表示受执行代码比例，不能代替断言质量与教学事实审校。

因此，README、页面和发布物不得使用“完全正确”“全部架构”“100% 覆盖”“零缺陷”“工业级复刻”等未经证据支持的认证性措辞。教学质量是按版本持续审计的属性，不是一次测试后永久成立的标签。

第一阶段历史记录见 [原交接清单](HANDOFF_SOL.md)；当前问题、修复顺序和验收断言以 [2026-08-23 第二阶段清单](HANDOFF_SOL_2026-08-23.md) 为准；长期不变量见 [项目记忆](PROJECT_MEMORY.md)。

## ✨ 学习路径与 M00-M17 里程碑总览

本项目目前提供 **M00-M17 共 18 个里程碑**。M17 当前处于“整改中/不可作为可信教学材料发布”；课程数量也不代表知识体系已经完整。完整状态见 [课程地图](CURRICULUM_MAP.md)：
...
| **工程可靠性篇** | M17 | 长上下文、Agent Harness 与工程失败模式 | **整改中**：现有模拟和事故指标存在证据错配，完成第二阶段清单前不作为可信教学结论 |
